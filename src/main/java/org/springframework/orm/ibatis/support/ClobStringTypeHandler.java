...
/**
 * iBATIS TypeHandler implementation for Strings that get mapped to CLOBs. Retrieves the LobHandler to use from
 * SqlMapClientFactoryBean at config time.
 * <p>
 * Particularly useful for storing Strings with more than 4000 characters in an Oracle database (only possible via
 * CLOBs), in combination with OracleLobHandler.
 * <p>
 * Can also be defined in generic iBATIS mappings, as DefaultLobCreator will work with most JDBC-compliant database
 * drivers. In this case, the field type does not have to be CLOB.
 *
